...
namespace {
	...
	class EloquentTypeHinting extends \Illuminate\Database\Eloquent\Model
    {
         ...
         *
		 * @return \Illuminate\Support\LazyCollection|static[]
         * @static
         */
		...
		/**
		 * Аналог chunkById с обратной сортировкой
		 *
		 * @param int $count Размер чанка
		 * @param callable $callback Callback-функция
		 * @param string|null $column Поле для сортировки
		 * @param string|null $alias Алиас
		 *
		 * @return bool
		 */
		public static function chunkByIdDesc(int $count, callable $callback, ?string $column = null, ?string $alias = null): bool {
			return true;
		}

		/**
		 * Выборка чанков сущностей (по умолчанию в обратном порядке) по идентификаторам связанных сущностей с лимитом
		 * Например выбрать 30 заказов по переданным идентификаторам кворков
		 *
		 * @param int $count Размер чанка выбираемых данных из БД
		 * @param int $limit Лимит выборки сущностей
		 * @param array $entityIds Идентификаторы связанных сущностей
		 * @param string $entityIdColumn Поле идентификатора связанной сущности
		 * @param string $column Поле идентификатора сущности
		 * @param callable $callback Callback-функция
		 * @param string $direction Направление сортировки идентификатора сущности
		 *
		 * @return bool
		 */
		public static function chunkByIdWithLimit(int $count, int $limit, array $entityIds, string $entityIdColumn, string $column, callable $callback, string $direction = "desc"): bool {
			return true;
		}
		 ...
		 * @param  string|null $as Алиас таблицы
		 *
		 * @return \Illuminate\Database\Eloquent\Builder|static
		 * @static
		 */
		...
		{
			return \Illuminate\Database\Eloquent\Builder::from($table, $as);
		}
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::join($table, $first, $operator, $second, $type, $where);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereRaw($sql, $bindings, $boolean);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereIn($column, $values, $boolean, $not);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::orWhereIn($column, $values);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereNotIn($column, $values, $boolean);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::orWhereNotIn($column, $values);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereNull($column, $boolean, $not);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::orWhereNull($column);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereNotNull($column, $boolean);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::orWhereBetween($column, $values);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::whereNotBetween($column, $values, $boolean);
        }
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::orWhereNotBetween($column, $values);
        }
         ...
         * @param bool $all
         * @return \Illuminate\Database\Query\Builder|\Illuminate\Database\Eloquent\Builder|static
         * @static
         */
        ...
        {
            return \Illuminate\Database\Eloquent\Builder::setBindings($bindings, $type);
        }
		...
		/**
		 * Безопасное удаление чанками, чтобы не было переполнения памяти и блокирования таблиц
		 *
		 * @param int $deleteChunkSize Сколько записей удалять за один раз
		 * @param int $selectChunkSize Сколько записей выбирать за один раз
		 * @param int $sleep Задержка в секундах вставляемая между итерациям (если нужно дать передышку для часто используемых таблиц)
		 *
		 * @return int Кол-во удаленных записей
		 */
		public static function deleteByChunk(int $deleteChunkSize = 10000, int $selectChunkSize = 100000, int $sleep = 0) {
			return 0;
		}
		...
		public function decrement($column, $amount = 1, array $extra = []) {
			return static::decrement($column, $amount, $extra);
		}

		/**
		 * Добавление/обновление записи
		 *
		 * @param array $attributes Массив атрибутов, по которым идет отбор записи
		 * @param array $values Массив значений, которые будут добавлены/обновлены по указанным атрибутам
		 */
		public static function insertIgnoreOrUpdate(array $attributes, array $values): int {
		}

		/**
		 * Вставка с игнорированием заданной ошибки СУДБ
		 *
		 * @param array $values Массив значений полей модели
		 *
		 * @return int|null
		 */
		public static function insertIgnoreError(array $values): ?int {
			return 0;
		}

		/**
		 * Вставка записей в базу данных с игнорированием ошибок
		 *
		 * @param array $values Массив вставляемых значений
		 * @deprecated Крайне не рекомендуется использовать запросы insert ignore, такие запросы приводят конфликтам в кластере БД, конфликты могу привести к развалу кластера
		 *
		 * @return int
		 */
...
namespace Fobia\Database\SphinxConnection\Eloquent {
	...
	use Model\Sphinx\SphinxBuilder;

	/**
	 * Добавление хинтинка в fobia билдер
	 * @package Fobia\Database\SphinxConnection\Eloquent\Query
	 */
	class Model {
		/**
		 ...
		 */
		public static function match($column, $value = null, $half = false) {
			return Query\Builder::match($column, $value, $half);
		}
		 ...
		 * @param string|array $value Значение опции
		 *
		 * @return Query\Builder|Builder
		 * @see Query\Builder::option
		 *
		 */
		public static function option(string $name, $value) {
			return Query\Builder::option($name, $value);
		}

		/**
		 * Replace a new record into the database.
		 *
		 * @param array $values Данные для вставки/замены
		 *
		 * @return bool
		 *
		...
		public static function replace(array $values): bool {
			return Query\Builder::replace($values);
		}

		/**
		 * Получить SphinxQL хэлпер
		 *
		 * @return Helper
		 */
		public static function getSphinxQLHelper(): Helper {
			return SphinxBuilder::getSphinxQLHelper();
		}

		/**
		 * Получить версию Sphinx
		 *
		 * @param int $precision Точность (1 - 3, 2 - 3.4, 3 - 3.4.1)
		 *
		 * @return string
		 */
		public static function getSphinxVersion(int $precision = 3): string {
			return SphinxBuilder::getSphinxVersion($precision);
		}

		/**
		 * Получить экранированную строку
		 *
		...
		public function escape(string $string): string {
			return SphinxBuilder::escape($string);
		}
	}
}
